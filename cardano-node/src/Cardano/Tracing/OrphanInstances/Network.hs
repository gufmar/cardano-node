...
instance Show peerAddr => ToJSON (ConnectionId peerAddr) where
  toJSON ConnectionId { localAddress, remoteAddress } =
    Aeson.object [ "localAddress"  .= show localAddress
...
instance Show addr => Aeson.ToJSONKey (ConnectionId addr) where
