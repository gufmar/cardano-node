...
import qualified Testnet.SubmitApi as H
...
hprop_plutus = H.integration . H.runFinallies . H.workspace "chairman" $ \tempAbsBasePath' -> do
  ...
  submitApiConfigFile <- H.note "TODO"
  submitApiStdoutFile <- H.note $ tempAbsPath </> "logs/submit-api.stdout"
  submitApiStderrFile <- H.note $ tempAbsPath </> "logs/submit-api.stderr"

  submitApiPort <- H.submitApi H.SubmitApiConfig
    { H.tempBaseAbsPath
    , H.base
    , H.configFile = submitApiConfigFile
    , H.sprocket = head bftSprockets
    , H.testnetMagic
    , H.stdoutFile = submitApiStdoutFile
    , H.stderrFile = submitApiStderrFile
    }
  ...
  result <- H.submitApiSubmitTransaction submitApiPort $ work </> "alonzo.tx"
